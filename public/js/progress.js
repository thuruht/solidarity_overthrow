function initializeProgressTrackers() {
    ...
    window.updateProgress = function(condition, value) {
        if (window.victoryConditions[condition]) {
            window.victoryConditions[condition].current = value;
...
function renderProgressTrackers() {
    ...
    for (const key in window.victoryConditions) {
        const condition = window.victoryConditions[key];
        let percentage;
        if (key === 'ipi') {
            // For IPI, progress is made as the value decreases
            percentage = Math.max(0, 100 * (100 - condition.current) / (100 - condition.target));
        } else {
...
// Example of how to update progress from another script:
// window.updateProgress('solidarity', 55);
// window.updateProgress('ipi', 70);