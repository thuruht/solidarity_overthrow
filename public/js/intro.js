...
function createSplashScreen() {
  ...
  subtitle.textContent = 'Unite the world against imperialism through collective action and revolutionary planning';
  ...
  startButton.innerHTML = '<span class="material-icons">play_arrow</span> Start Game';
  startButton.addEventListener('click', startGame);
  
  // Tutorial button
  const tutorialButton = document.createElement('button');
  tutorialButton.className = 'splash-button';
  tutorialButton.innerHTML = '<span class="material-icons">school</span> Tutorial';
...
function createTutorialSlides() {
  ...
  slides.forEach((slide, index) => {
    ...
    if (index === slides.length - 1) {
      nextButton.innerHTML = 'Start Game <span class="material-icons">play_arrow</span>';
...
function createHelpPanel() {
  ...
  tabTitles.forEach((tabTitle, index) => {
    ...
    tab.addEventListener('click', (e) => {
      ...
      document.getElementById(`help-content-${e.target.dataset.tab}`).classList.add('active');
    });
